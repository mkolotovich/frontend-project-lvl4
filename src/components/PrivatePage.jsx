...
function Home(props) {
  const { params } = props;
  const {
    channelId, handleClose, handleShow, channelSwitchHandler, channelName,
  } = params;
  const modal = { add: 0, remove: 1, rename: 2 };
  ...
  const [add, remove, rename] = allModals;
  useEffect(() => {
    const request = async () => {
  ...
  return (
    <div className="container my-4 overflow-hidden h-100 shadow rounded">
      <div className="row h-100 bg-white">
        <div className="col-4 col-md-2 border-end pt-5 px-0 bg-light">
          <div className="d-flex justify-content-between mb-2 ps-4 pe-2">
            {t('channels')}
            <button className="btn btn-group-vertical p-0 text-primary" onClick={(evt) => handleShow(evt, modal.add)} type="button">
            ...
            {allChannels.map((channel) => (
              <li className="w-100" key={channel.id}>
                ...
                <Dropdown as={ButtonGroup} className="d-flex">
                  ...
                  <Dropdown.Menu>
                    <Dropdown.Item href="#/action-1" onClick={(evt) => handleShow(evt, modal.remove, channel.id, allChannels)}>{t('remove')}</Dropdown.Item>
                    <Dropdown.Item href="#/action-2" onClick={(evt) => handleShow(evt, modal.rename, channel.id, allChannels)}>{t('rename')}</Dropdown.Item>
                  </Dropdown.Menu>
                </Dropdown>
                )}
              </li>
      ...
      <RemoveChannelModal modal={remove} handle={handleClose} id={channelId} />
      <RenameChannelModal modal={rename} handle={handleClose} id={channelId} name={channelName} />
    </div>
  );
}
